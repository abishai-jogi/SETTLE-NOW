 ...
 * (sorted user ids), so every device computes identical shares.
 */
object SplitCalculator {
    ...
    fun equalShares(totalCents: Long, participantIds: List<String>): ValidationResult =
        ...
        } else Valid(equalSharesCents(totalCents, participantIds))

    fun equalSharesCents(totalCents: Long, participantIds: List<String>): List<Pair<String, Long>> {
        val ids = participantIds.distinct().sorted()
        val n = ids.size
        if (n == 0 || totalCents <= 0) return ids.map { it to 0L }
        val base = totalCents / n
        val remainder = totalCents % n
        return ids.mapIndexed { index, id ->
            id to (base + if (index < remainder) 1L else 0L)
        }
    }
        ...
        return if (sum == totalCents) {
            ...
            Invalid(
                "Assigned ${Money.formatCents(sum)} of ${Money.formatCents(totalCents)} — " +
                    if (sum < totalCents) "${Money.formatCents(totalCents - sum)} left"
                    else "${Money.formatCents(sum - totalCents)} over"
            )
        }
    }
    ...
    fun liveShares(expenses: List<Expense>): List<Expense> =
