...
interface ExpenseDao {
    ...
    @Transaction
    @Query(
        "SELECT * FROM expenses WHERE ledger_id = :ledgerId AND is_deleted = 0 ORDER BY created_at DESC"
    )
    fun observeByLedger(ledgerId: String): Flow<List<ExpenseWithParticipants>>
